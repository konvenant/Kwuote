...
class SearchAuthorsPagingSource @Inject constructor (
...
) : PagingSource<Int, SearchAuthorResult>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SearchAuthorResult> {
        val position = params.key ?: 1
        return try {
            val quotes = quoteApi.searchAuthors(query,position)
            LoadResult.Page(
                data = quotes.results,
    ...
    override fun getRefreshKey(state: PagingState<Int, SearchAuthorResult>): Int? {
